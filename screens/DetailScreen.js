...
/* ---------------- Helper KeyValue Component for rows ---------------- */
...
/* ---------------- Modal for Reject Reason with dynamic dropdown ---------------- */
...
/* ---------------- Modal for Approve Comment Modal ---------------- */
...
const DetailScreen = () => {
  ...
  const [tfImages, setTFImages] = useState([]);
  ...
  useEffect(() => {
  const fetchTFImages = async () => {
    try {
      const response = await fetch('http://10.0.2.2:3001/api/tf-images');
      const result = await response.json();
      if (result.success) {
        setTFImages(result.data);
      }
    } catch (error) {
      console.error('TF image fetch failed', error);
    }
  };
  ...
  return (
    ...
    <ScrollView style={styles.container}>
      ...
      <CollapsibleCard
  title="TF Images"
  isExpanded={expandedCard === 'TF Images'}
  onToggle={() => setExpandedCard(expandedCard === 'TF Images' ? null : 'TF Images')}
>
  <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
    {tfImages.map((img, idx) => (
      <View key={idx} style={{ width: '48%', marginBottom: 12 }}>
        {img.url ? (
          <Image
            source={{ uri: img.url }}
            style={{ width: '100%', height: 120, borderRadius: 10, backgroundColor: '#000' }}
            resizeMode="cover"
          />
        ) : (
          <View style={{
            ...
            <Text style={{ color: '#333' }}>No Image Available</Text>
          </View>
        )}
        <Text style={{ textAlign: 'center', marginTop: 4 }}>{img.label}</Text>
      </View>
    ))}
  </View>
</CollapsibleCard>
        ...
        {historyLoading ? (
          ...
          historyLogs.map((log, index) => (
            <View key={index} style={{ marginBottom: 12 }}>
              <Text style={{ color: '#3d5596ff', fontSize: 16, fontWeight: 'bold' }}>
